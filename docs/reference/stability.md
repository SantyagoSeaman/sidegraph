...
| **CLI commands, flags, exit codes** | 17 entry points, 73 flags. A flag is not removed without a deprecation window; documented exit codes keep their meaning | every parser option must appear in its `cli.md` section (`tests/test_docs_claims.py`) |
